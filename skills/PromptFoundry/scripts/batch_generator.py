...
class BatchGenerator:
    ...
    def load_json_batch(self, filepath: str) -> list[dict[str, Any]]:
        ...
        if isinstance(data, list):
            ...
            raise ValueError(
                "JSON must be array of configs or object with 'prompts' key"
            )
    ...
    def generate_single(
        ...
        try:
            ...
            # Validation summary
            validation_summary = {
                fmt: val["passed"] for fmt, val in result["validation"].items()
            }
    ...
    def generate_batch(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(
                    self.generate_single, config, format_type, mode, output_dir
                ): config
...
""",
    ...
    if input_path.suffix == ".csv":
        print("📄 Loading CSV batch configuration...")
        configs = batch_gen.load_csv_batch(args.input)
    elif input_path.suffix == ".json":
        print("📄 Loading JSON batch configuration...")
