...
function containsLegacyIdentifier(text) {
  return Object.keys(LEGACY_TO_CANONICAL).some((legacy) =>
...
function replaceIdentifiers(content) {
  ...
  const sortedLegacy = Object.keys(LEGACY_TO_CANONICAL).sort(
...
function processFile(filePath) {
  ...
  if (
    filePath.endsWith("rename-skills.js") ||
    filePath.endsWith("update-skill-identifiers.yml")
  ) {
    return;
  }
