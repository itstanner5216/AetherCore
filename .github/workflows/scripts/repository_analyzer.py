...
class RepositoryAnalyzer:
    ...
    IGNORE_DIRS = {
        ...
        'quarantine', '.github/workflows/scripts'  # Don't analyze ourselves
    }
    ...
    ENTRY_POINTS = {
        'main.py', 'index.js', 'server.js', 'app.py', 'gateway.py',
        'index.ts', 'main.js', 'setup.py', 'manage.py', '__main__.py',
    ...
    def __init__(self, repo_path: str, output_dir: str = None):
        ...
        self.output_dir = Path(output_dir) if output_dir else self.repo_path / '.github' / 'cleanup_reports'
    ...
    def _identify_quarantine_candidates(self) -> List[Dict]:
        ...
        for path, analysis in self.file_analyses.items():
            ...
            if any(marker in file_name for marker in ['_old', '_backup', '_copy', '.bak', '_deprecated']):
        ...
        for link in semantic_results.get('code_doc_links', []):
            if link.get('type') == 'doc_without_code':
                ...
                inconsistencies.append({
                    ...
                    'description': f"Code file has no corresponding documentation"
                })
    ...
    def save_reports(self) -> Tuple[Path, Path]:
        ...
        with open(json_path, 'w') as f:
            ...
            report_dict['files'] = {
                k: asdict(v) for k, v in self.report.files.items()
            }
