...
class RepositoryAnalyzer:
    ...
    def __init__(self, repo_path: str, output_dir: str = None):
        ...
        self.output_dir = (
            Path(output_dir)
            if output_dir
            else self.repo_path / ".github" / "cleanup_reports"
        )
        ...
        self.report: RepositoryReport | None = None
        ...
        for item in self.repo_path.rglob("*"):
            if item.is_file():
                ...
                if (
                    ...
                    or item.name in self.ENTRY_POINTS
                ):
                    files.append(item)
    ...
    def classify_file(self, file_path: Path) -> str:
        ...
        elif ext in self.CONFIG_EXTENSIONS or name in {
            "dockerfile",
            "procfile",
            "makefile",
        }:
...
def main():
    ...
    parser.add_argument(
        "--quarantine", "-q", action="store_true", help="Move files to quarantine"
    )
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="Dry run (no file moves)"
    )
    parser.add_argument(
        "--adversarial",
        "-a",
    ...
    output_dir = (
        Path(args.output) if args.output else repo_path / ".github" / "cleanup_reports"
    )
    ...
    if use_adversarial:
        ...
        court = FileCourt(
            repo_root=str(repo_path), conservative=args.conservative, verbose=True
        )
