...
class SemanticAnalyzer:
    ...
    STOP_WORDS = {
        ...
        'then', 'once', 'if', 'else', 'elif', 'while', 'return', 'import',
        'from', 'def', 'class', 'function', 'const', 'let', 'var', 'true',
        'false', 'none', 'null', 'undefined', 'async', 'await', 'try', 'catch',
        'except', 'finally', 'throw', 'raise', 'new', 'self', 'this'
    }
    
    # Common code tokens to filter
    CODE_NOISE = {
        'str', 'int', 'float', 'bool', 'list', 'dict', 'set', 'tuple',
        'array', 'object', 'string', 'number', 'boolean', 'any', 'void',
        'type', 'interface', 'enum', 'export', 'default', 'public', 'private',
        'protected', 'static', 'readonly', 'abstract', 'extends', 'implements'
    }
    ...
    def _analyze_javascript(self, content: str, semantics: FileSemantics):
        ...
        full_text = f"{' '.join(jsdocs)} {' '.join(comments)} {' '.join(semantics.entities)}"
        semantics.word_freq = self._extract_word_freq(full_text)
    ...
    def _extract_word_freq(self, text: str) -> Dict[str, int]:
        ...
        words = [w for w in words if w not in self.STOP_WORDS and w not in self.CODE_NOISE]
    ...
    def _find_clusters(self) -> List[Dict]:
        ...
        sorted_files = sorted(
            self.file_semantics.items(),
            key=lambda x: len(x[1].topics),
            reverse=True
        )
        ...
        for path, semantics in sorted_files:
            ...
            if len(cluster_files) > 1:
                ...
                main_topic = topic_counts.most_common(1)[0][0] if topic_counts else 'misc'
    ...
    def _link_code_to_docs(self) -> List[Dict]:
        ...
        code_files = {p: s for p, s in self.file_semantics.items() if s.file_type == 'code'}
        doc_files = {p: s for p, s in self.file_semantics.items() if s.file_type == 'documentation'}
        ...
        for doc_path, doc_sem in doc_files.items():
            ...
            for code_path, code_sem in code_files.items():
                ...
                entity_mentions = sum(
                    1 for e in code_sem.entities
                    if e.lower() in ' '.join(doc_sem.word_freq.keys())
                )
                
                score = (0.3 if name_match else 0) + (similarity * 0.5) + (min(entity_mentions, 5) * 0.04)
    ...
    def _generate_warnings(self) -> List[str]:
        ...
        if empty_files:
            warnings.append(f"Found {len(empty_files)} files with no extractable content")
