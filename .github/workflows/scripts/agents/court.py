...
class FileCourt:
    ...
    def __init__(self, repo_root: str, conservative: bool = True, verbose: bool = True):
        ...
        if self.verbose:
            print("⚖️  FILE COURT INITIALIZED")
    ...
    def run_trial(self, file_path: str) -> TrialRecord:
        ...
        if self.verbose:
            print(
                f"   Verdict: {prosecution_case.verdict} ({prosecution_case.confidence:.0%})"
            )
        ...
        if self.verbose:
            print("\n⚖️  JUDGMENT:")
        verdict = self.judge.judge(prosecution_case.to_dict(), defense_case.to_dict())
        if self.verbose:
    ...
    def run_all_trials(self, suspects: list[str] | None = None) -> list[TrialRecord]:
    ...
    def get_verdicts_by_decision(self) -> dict[str, list[str]]:
        ...
        by_decision = {"KEEP": [], "QUARANTINE": [], "DELETE": [], "REVIEW_NEEDED": []}
    ...
    def generate_full_report(self) -> str:
        ...
        report.append("╔" + "═" * 68 + "╗")
        report.append(
            "║" + "⚖️  FILE COURT - ADVERSARIAL ANALYSIS REPORT".center(68) + "║"
        )
        report.append("╚" + "═" * 68 + "╝")
        report.append("")
        ...
        report.append("├" + "─" * 50 + "┤")
        report.append(
            f"│  ✅ KEEP:           {len(verdicts['KEEP']):4d} files".ljust(51) + "│"
        )
        report.append(
            f"│  📦 QUARANTINE:     {len(verdicts['QUARANTINE']):4d} files".ljust(51)
            + "│"
        )
        report.append(
            f"│  🗑️  DELETE:         {len(verdicts['DELETE']):4d} files".ljust(51) + "│"
        )
        report.append(
            f"│  👀 REVIEW NEEDED:  {len(verdicts['REVIEW_NEEDED']):4d} files".ljust(51)
            + "│"
        )
        report.append("└" + "─" * 50 + "┘")
        report.append("")
        ...
        if verdicts["QUARANTINE"]:
            ...
            for trial in self.trials:
                if trial.verdict.get("decision") == "QUARANTINE":
                    ...
                    # Show key reasoning
                    key_factors = trial.verdict.get("reasoning", {}).get(
                        "key_factors", []
                    )
                    if key_factors:
        ...
        if verdicts["KEEP"]:
            ...
            report.append(
                "║"
                + f" ✅ FILES CONFIRMED SAFE ({len(verdicts['KEEP'])} files)".ljust(68)
                + "║"
            )
            report.append("╚" + "═" * 68 + "╝")
            # Show top 10 with highest defense scores
            kept_trials = [
                t for t in self.trials if t.verdict.get("decision") == "KEEP"
            ]
            kept_trials.sort(
            ...
            for trial in kept_trials[:10]:
                def_weight = (
                    trial.verdict.get("reasoning", {}).get("defense_weight", 0) * 100
                )
        ...
        for trial in self.trials:
            if trial.verdict.get("decision") in [
                "QUARANTINE",
                "REVIEW_NEEDED",
                "DELETE",
            ]:
                ...
                report.append(
                    f"🔴 PROSECUTION: {pros.get('verdict')} ({pros.get('confidence', 0):.0%})"
                )
                ...
                report.append(
                    f"🟢 DEFENSE: {defe.get('verdict')} ({defe.get('confidence', 0):.0%})"
                )
                ...
                verd = trial.verdict
                report.append(
                    f"⚖️  VERDICT: {verd.get('decision')} ({verd.get('confidence', 0):.0%})"
                )
    ...
    def get_action_items(self) -> dict[str, list[dict]]:
        ...
        for trial in self.trials:
            ...
            item = {
                ...
                "reasoning": trial.verdict.get("reasoning", {}).get("key_factors", [])[
                    :2
                ],
            }
            ...
            elif decision == "REVIEW_NEEDED" or (
                decision == "QUARANTINE" and confidence < 0.8
            ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="File Court - Adversarial File Analysis"
    )
    ...
    # Initialize court
    court = FileCourt(
        repo_root=args.repo_root, conservative=conservative, verbose=verbose
    )
