...
class JudgeAgent:
    ...
    HIGH_RISK_PATTERNS = [
        'auth', 'security', 'config', 'main', 'index', 'server', 
        'app', 'core', 'base', 'util', 'helper', 'api', 'gateway',
        '__init__', 'setup', 'install', 'deploy', 'build'
    ]
    ...
    def judge(self, prosecution_case: Dict, defense_case: Dict) -> Verdict:
        ...
        dissenting_points = self._identify_dissenting_points(
            ...
            prosecution_weight > defense_weight
        )
        
        # Assess risk
        risk_assessment = self._assess_risk(
            file_path, prosecution_case, defense_case, is_high_risk
        )
        
        # Make decision
        decision, confidence = self._make_decision(
            prosecution_weight, defense_weight,
            is_high_risk, dissenting_points
        )
    ...
    def _is_high_risk(self, file_path: str) -> bool:
        ...
        return any(
            pattern in filename or pattern in stem 
    ...
    def _identify_dissenting_points(
        self, prosecution: Dict, defense: Dict, 
        prosecution_won: bool
    ) -> List[str]:
        ...
        for evidence in losing_case.get('evidence', []):
            if evidence.get('strength', 0) >= 0.7:
                dissent.append(
                    f"{'⚠️' if prosecution_won else '⚡'} {evidence.get('description')}"
                )
    ...
    def _make_decision(
        self, prosecution_weight: float, defense_weight: float,
        is_high_risk: bool, dissenting_points: List[str]
    ) -> Tuple[str, float]:
    ...
    def _generate_recommendation(self, file_path: str, decision: str, reasoning: JudgmentReasoning) -> str:
        ...
        if decision == "QUARANTINE":
            rec = f"📦 QUARANTINE `{filename}` - Move to quarantine folder for observation period.\n"
...
if __name__ == "__main__":
    ...
    mock_prosecution = {
        ...
        "confidence": 0.7,
        "evidence": [
            {"category": "NO_IMPORTS", "description": "File is not imported anywhere", "strength": 0.8},
