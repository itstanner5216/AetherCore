...
class ReasoningChain {
  ...
  decompose(query, maxSteps = 5) {
    ...
    if (words.some(w => ['compare', 'vs', 'versus', 'difference'].includes(w))) {
      steps = [
    ...
    const dependencies = {};
    result.forEach(s => { dependencies[s.id] = s.depends_on || []; });
  ...
  synthesize(steps, originalQuery) {
    ...
    const avgConfidence = trace.length > 0
...
module.exports = {
  ...
  reason_step: (params) => engine.reasonStep(params.step_id, params.question, params.context),
  verify_step: (params) => engine.verifyStep(params.step_id, params.conclusion),
  synthesize: (params) => engine.synthesize(params.steps, params.original_query),
